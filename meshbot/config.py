...
from meshbot.models import BotConfig, MessageConfig
...
def _build_config(data: dict[str, Any]) -> BotConfig:
    ...
    valid_fields = {f.name for f in BotConfig.__dataclass_fields__.values()} - {"message"}
    filtered = {k: v for k, v in data.items() if k in valid_fields}

    return BotConfig(message=message, **filtered)
